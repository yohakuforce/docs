...
> - 環境: 社内に常時起動の Mac mini が 1 台、各自は MacBook、チャットは Slack
...
```mermaid
flowchart TB
    subgraph host["🖥 共有ホスト1台（社内 Mac mini・常時起動）"]
...
| **core（`yohaku`）** | 各開発者の MacBook | 設計書生成は手元で。共有 context-hub を参照 |
...
共有 Mac mini で（`production` プロファイル・PostgreSQL）:

```bash
pipx install "yohakuforce-context-hub[embedding]"
cd ~/yohakuforce/context-hub      # 作業フォルダ
