...
## Context-Hub ではないもの（誤解の整理）

- **チャット UI ではありません** — 会話は Claude 等の MCP クライアント側で行います。Context-Hub は文脈の保管・提供に徹します。
- **マシン全体の検索エンジンではありません** — 取り込んだものしか知りません。
- **バックアップではありません** — SQLite を消すと全消失します。`data/` と `.env` をセットでバックアップしてください。
- **マルチテナント SaaS ではありません** — API キーはインスタンス単位。信頼境界ごとに別インスタンスを立てます。

## 誤って取り込んだときの対処

1. サーバを停止する
2. `GET /api/v1/projects/{id}/context` 等で対象を特定する
3. SQLite から該当を削除、または `rm -rf data/` で全リセットする
4. Gmail 由来ならラベルを外して再同期し、カーソルを追従させる

!!! note "状態の在り処"
    ドキュメント / プロジェクト / ジョブ＝`./data/context_hub.db`、スケジューラ＝`./data/scheduler.db`、Gmail トークン＝`$GMAIL_TOKEN_FILE`、API キー＝`.env`。ログは structlog による構造化 JSON で stdout に出力されます（`context-hub serve 2>&1 | jq -R 'fromjson? // .'` で整形可）。
