...
日報テンプレは固定様式ではなく、その日の担当タスク状態から動的に組み立てられます。

| 質問型 | 内容 | 付与条件 |
|---|---|---|
| `progress_percent`（進捗率） | 「〈タスク名〉の進捗率（0〜100）を入力」 | 担当タスクごとに 1 問 |
| `blocker`（ブロッカー） | 「〈タスク名〉にブロッカーがあれば記入（なければ『なし』）」 | 担当タスクごとに 1 問 |
| `customer_pending`（顧客確認） | 「ブロック解除に顧客への確認が必要か」 | ブロック中タスクのみ追加 |
| `free_text`（自由記述） | 担当ありなら「その他の共有事項」、担当なしなら「本日の作業内容と成果」 | 常に末尾に 1 問 |

#### 日報のステータス遷移

`pending → delivered → submitted → analyzed`

| 状態 | 意味 |
|---|---|
| `pending` | テンプレ生成済み・未配信 |
| `delivered` | 配信済み（未回答なら催促・アラートの対象） |
| `submitted` | メンバーが回答を送信 |
| `analyzed` | AI が解析しサマリ・ブロッカーを付与 |

!!! note "二重生成しない"
    当日分が既にあるメンバーはスキップされます。配信は各メンバーの `external_id` を宛先（Slack DM / チャネル等）として解決し、未設定なら既定チャネルへフォールバックします。配信先の形式は [出力先（通知チャネル）](notifications.md) を参照。
...
| カテゴリ | 検知条件 | 重要度 |
|---|---|---|
| `task_delay`（タスク遅延） | 期日超過（即時） | 3 日以上=critical / 1 日以上=high / 当日=medium |
| `member_overload`（過負荷） | 未完了の確認済みタスクが 5 件以上 | high |
| `customer_no_response`（日報未回答） | 配信済みのまま N 日（既定 1 日）未回答 | medium |

- **重複抑止**: 同じ対象に既存のアクティブアラートがあれば再生成しません。
- **状態管理**: `active → acknowledged（確認）→ resolved（解決）`。必ず「なぜ鳴ったか」の根拠（evidence）を添えます。
